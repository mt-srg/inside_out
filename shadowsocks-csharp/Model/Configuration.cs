...
namespace Shadowsocks.Model
{
    ...
    public class Configuration
    {
        ...
        {
            //System.Diagnostics.Debug.WriteLine("keep current server");
        ...
        {
            //System.Diagnostics.Debug.WriteLine("get current server");
            System.Diagnostics.Debug.WriteLine(targetAddr);
            //if client is in private pac mode
            if (pacPrivate)
            {
                //select server by server remarks
                string _server_remarks = "";
                foreach (KeyValuePair<string,string> entry in PACPrivateServer.PrivateRules)
                {
                    if (targetAddr.Contains(entry.Key))
                    {
                        _server_remarks = entry.Value;
                        System.Diagnostics.Debug.WriteLine(_server_remarks);
                        int _server_index=configs.FindIndex(server=>server.remarks==_server_remarks); 
                        System.Diagnostics.Debug.WriteLine(_server_index);
                        if (_server_index!=-1)
                        {
                            return configs[_server_index];
                        }
                    }
                }
            }
