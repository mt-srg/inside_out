...
namespace Shadowsocks.Controller
{
    public class GFWListUpdater
    {
        ...
        private static string PAC_PRIVATE_FILE = PACServer.PAC_PRIVATE_FILE;
        ...
        private void http_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            try
            {
                ...
                // load current config
                Configuration _currentConfig = Configuration.Load();
                System.Diagnostics.Debug.WriteLine(_currentConfig.pacPrivate);
                if (_currentConfig.pacPrivate)
                {
                    if (File.Exists(PAC_PRIVATE_FILE))
                    {
                        string local = File.ReadAllText(PAC_PRIVATE_FILE, Encoding.UTF8);
                        string[] rules = local.Split(new char[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
                        foreach(string rule in rules)
                        {
                            ...
                            lines.Add(rule.Split(' ')[0].Insert(0,"||"));
                            System.Diagnostics.Debug.WriteLine(rule.Split(' ')[0].Insert(0,"||"));
                        }
                    }
                }
        ...
        public void UpdatePACFromGFWList(Configuration config)
        {
            if (gfwlist_template == null)
            {
                ...
                System.Diagnostics.Debug.WriteLine(GFWLIST_TEMPLATE_URL);
            ...
            {
                System.Diagnostics.Debug.WriteLine(GFWLIST_URL);
                WebClient http = new WebClient();
                WebProxy proxy = new WebProxy(IPAddress.Loopback.ToString(), config.localPort);
                if (!string.IsNullOrEmpty(config.authPass))
                {
